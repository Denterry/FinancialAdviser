...
// ConversationUseCase defines the business logic for conversation management
type ConversationUseCase interface {
	// CreateConversation starts a new conversation
	CreateConversation(ctx context.Context, userID, title string) (*entity.Conversation, error)

	// GetConversation retrieves a conversation by ID
	GetConversation(ctx context.Context, id string) (*entity.Conversation, error)

	// ListConversations retrieves conversations based on filter criteria
	ListConversations(ctx context.Context, filter repo.ConversationFilter) ([]*entity.Conversation, error)

	// DeleteConversation removes a conversation
	DeleteConversation(ctx context.Context, id string) error

	// SendMessage adds a new message to the conversation and processes it
	SendMessage(ctx context.Context, convID string, content string) (*entity.Message, error)

	// GetMessage retrieves a specific message
	GetMessage(ctx context.Context, convID, msgID string) (*entity.Message, error)
}

// LLMProvider defines the interface for LLM service integration
type LLMProvider interface {
	// GenerateResponse generates a response using the LLM
	GenerateResponse(ctx context.Context, messages []entity.Message) (string, error)
